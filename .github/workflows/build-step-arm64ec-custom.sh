  ...
  then
    ...
    if [ -d "$PROJECT_ROOT/android/android_sysvshm" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # 验证文件是否存在
            if [ -f "$deps/lib/libandroid-sysvshm.so" ]; then
                echo "libandroid-sysvshm.so verified in $deps/lib"
            else
                echo "ERROR: libandroid-sysvshm.so not found after copy"
                exit 1
            fi
        else
            echo "ERROR: android_sysvshm build failed"
            exit 1
        fi
        cd "$PROJECT_ROOT"
    else
        echo "ERROR: android_sysvshm directory not found"
        exit 1
    fi
  fi
...
#      "test-bylaws/tools_makedep_c.patch"
  ...
  then
    ...
    for lib in libandroid-sysvshm.so libpulse.so libSDL2.so libfreetype.so; do
        if [ -f "$deps/lib/$lib" ]; then
            echo "  $lib: found"
        else
            echo "  $lib: NOT FOUND (may cause link errors)"
        fi
    done
    ...
    if [ -f "./wine" ]; then
        SIZE=$(stat -c %s ./wine)
        echo "wine binary size: $SIZE bytes"
        if [ $SIZE -lt 1000000 ]; then
            echo "ERROR: wine binary is too small (likely link failure)"
            echo "Check if all libraries are present and link command succeeded."
  ...
  then
    ...
    echo "Binary listing:"
    ls -lh "$OUTPUT_DIR$install_dir/bin"
  fi
done